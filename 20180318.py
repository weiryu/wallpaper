...
打包程序 pyinstaller -F -c -w -i title.ico 20180318.py
注释：使用anaconda python 3.5环境 pyinstaller 3.0打包，其他环境可能出现问题
"""
...
class Window(QMainWindow, Ui_MainWindow):
    ...
    def about_software_information(self):
        QMessageBox.about(self, "关于软件", "Version 1.3.3\nCopyright@weir\nAll rights reserved")
    ...
    def nationalgeographic_wallpaper(self):
        ...
        if urllist!= []:
            ...
            if imglist != []:
                ...
                m = int(10000*(time.localtime(time.time())[0])+100*(time.localtime(time.time())[1])+time.localtime(time.time())[2])
                filetype = imglist[len(imglist) - 1].split(".")[-1]
                img_save_url = None
                for i in range(0,100):
                    for img_list_name in imglist:
                        pattern = re.compile(str(m-i))
                        if pattern.findall(img_list_name):
                            img_save_url = img_list_name
                            break
                        else:
                            pass
                    if img_save_url !=None:
                        break
                    else:
                        pass                     
                filename = 'national-geographic' + str(m-i) + "." + str(filetype)
                ...
                if os.path.isfile(local_nationalgeographic + "\\" + filename):
                    ...
                    urllib.request.urlretrieve(img_save_url, local_nationalgeographic + "\\" + filename)
                # 修改当天更换过壁纸后无法再次更换的bug
